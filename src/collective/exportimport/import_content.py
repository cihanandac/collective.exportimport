...
class ImportContent(BrowserView):
    ...
    def import_new_content(self, data):  # noqa: C901
        ...
        for index, item in enumerate(data, start=1):
            ...
            logger.debug("Checking existing content for item with id: %s", item["id"])
            if item["id"] in container:
                existing = container[item["id"]]
                logger.debug("Found existing item by id: %s", item["id"])
            else:
            ...
            if existing:
                if self.handle_existing_content == 0:
                    ...
                    api.content.delete(existing, check_linkintegrity=False)
            ...
            try:
                ...
                logger.warning(e)
                logger.warning(
                    "Didn't add {} {}".format(item["@type"], item["@id"]), exc_info=True
                )
                continue
